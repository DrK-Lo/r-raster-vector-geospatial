...
> >  [17] Arith          as.array       as.character   as.data.frame 
> >  [21] as.integer     as.list        as.logical     as.matrix     
> >  [25] as.vector      atan2          bbox           boxplot       
> >  [29] brick          calc           cellFromXY     cellStats     
> >  [33] clamp          click          coerce         colFromX      
> >  [37] colSums        Compare        coordinates    corLocal      
> >  [41] cover          crop           crosstab       cut           
> >  [45] cv             density        dim            dim<-         
> >  [49] disaggregate   dropLayer      extend         extent        
> >  [53] extract        flip           freq           getValues     
> >  [57] getValuesBlock getValuesFocal head           hist          
> >  [61] image          interpolate    intersect      is.factor     
> >  [65] is.finite      is.infinite    is.na          is.nan        
> >  [69] isLonLat       KML            labels         length        
> >  [73] levels         levels<-       log            Logic         
> >  [77] mask           match          Math           Math2         
> >  [81] maxValue       mean           merge          minValue      
> >  [85] modal          mosaic         names          names<-       
> >  [89] ncell          ncol           nlayers        nrow          
> >  [93] origin         origin<-       overlay        pairs         
> >  [97] persp          plot           plotRGB        predict       
> > [101] print          proj4string    proj4string<-  quantile      
> > [105] raster         rasterize      readAll        readStart     
> > [109] readStop       reclassify     res            resample      
> > [113] rotate         rowFromY       rowSums        sampleRandom  
> > [117] sampleRegular  scale          select         setMinMax     
> > [121] setValues      shift          show           spplot        
> > [125] stack          stackSelect    subs           subset        
> > [129] Summary        summary        t              tail          
> > [133] text           trim           unique         unstack       
> > [137] values         values<-       weighted.mean  which.max     
> > [141] which.min      whiches.max    whiches.min    writeRaster   
> > [145] xFromCell      xFromCol       xmax           xmin          
> > [149] xres           xyFromCell     yFromCell      yFromRow      
